...
# KONFIGURACE STRÁNKY
# Nastavení základního rozvržení aplikace, názvu karty v prohlížeči a ikony.
st.set_page_config(
...
# Pomocná funkce pro sjednocení čištění časových řad z Českého statistického úřadu
def prep_timeseries(df, date_col):
    """
    Zajišťuje standardizaci datových formátů: přejmenování sloupce s datem, 
    odstranění zástupných znaků pro prázdné hodnoty, nastavení časového indexu 
    a konverzi textových čísel na standardní datový typ float.
    """
...
# 1. NAČÍTÁNÍ ZÁKLADNÍCH DAT (CPI A HICP)
# Využíváme dekorátor @st.cache_data, aby aplikace nenačítala Excelové soubory 
# z disku při každé interakci uživatele, což výrazně zrychluje odezvu.
@st.cache_data
def load_inflation_data():
    try:
        # --- Národní index spotřebitelských cen (CPI) ---
        # Kvůli změně metodiky ČSÚ je nutné spojit dvě oddělené časové řady (starou a novou).
        ...
        # Výpočet dynamiky vývoje cen (meziroční a meziměsíční tempo)
        cpi_yoy = cpi_raw.pct_change(12) * 100
        cpi_mom = cpi_raw.pct_change(1) * 100

        # --- Harmonizovaný index spotřebitelských cen (HICP - metodika EU) ---
        hicp_raw = pd.read_excel("HICP.xlsx")
        hicp_raw = prep_timeseries(hicp_raw, "Oddíl COICOP")
        hicp_yoy = hicp_raw.pct_change(12) * 100

        # Pro zachování konzistence omezíme obě časové řady pevným počátečním datem
        ...
        # Ošetření chyby pro případ, že zdrojové soubory chybí nebo mají poškozenou strukturu
        st.error(f"Kritická chyba při načítání dat o inflaci: {e}")
        return None, None, None, None, None


# 2. NAČÍTÁNÍ DAT O STRUKTUŘE SPOTŘEBNÍHO KOŠE (VÁHY)
@st.cache_data
def load_weights_data():
    # A) Detailní složení koše pro mapu (Treemap)
    try:
        weights_tree = pd.read_excel('spot_kos2025_podrobne.xlsx')
        
        # Konverze desetinných čárek a převod promile na procenta pro snazší interpretaci
        ...
        # Převodník klasifikačních kódů na uživatelsky přívětivé názvy kategorií
        category_names = {
            '01': 'Potraviny a nealko nápoje', '02': 'Alkohol a tabák', '03': 'Odívání a obuv',
            '04': 'Bydlení, voda, energie', '05': 'Bytové vybavení', '06': 'Zdraví',
        ...
        # Izolace dvoumístného hlavního kódu oddílu (např. z 01.1.2 udělá 01)
        ...
        # Pro vizualizaci stromové mapy filtrujeme pouze podrobnější položky (obsahující tečku v kódu)
    ...
    # B) Historický vývoj vah koše v průběhu let
    try:
        weights_history = pd.read_excel('Vahy_v_letech.xlsx')
        weights_history.columns = weights_history.columns.astype(str)
        
        # Odstranění souhrnného řádku, protože pro vizualizaci strukturálních změn potřebujeme jen dílčí části
        weights_history = weights_history[weights_history['NAZEV'] != 'ÚHRN'].copy()
        
        # Dynamická identifikace sloupců obsahujících jednotlivé roky a aplikace matematických úprav
...
# Inicializace primárních datových sad
cpi_raw, cpi_yoy, cpi_mom, hicp_raw, hicp_yoy = load_inflation_data()
weights_tree, weights_history = load_weights_data()

# Zastavení běhu aplikace, pokud selže načtení kritických dat
if cpi_raw is None: 
    st.stop()


# --- 3. UŽIVATELSKÉ ROZHRANÍ - BOČNÍ PANEL (SIDEBAR) ---
st.sidebar.title("⚙️ Nastavení")

# A) Výběr analyzovaného období (Fyzický filtr datových řad)
st.sidebar.subheader("1. Načíst období dat")
st.sidebar.caption("Určete časový úsek, pro který se mají spočítat statistiky.")
...
# Validace chronologické posloupnosti zadaných dat
if filter_start > filter_end:
    st.error("Logická chyba: Počáteční datum ('Načíst OD') nesmí být po koncovém datu ('Načíst DO').")
    st.stop()

# Vytvoření oříznutých datových rámců, které budou sloužit jako základ pro všechny výpočty
...
# B) Uživatelské přiblížení grafů (Pouze vizuální zoom, nemění samotné výpočty)
st.sidebar.subheader("2. Pohled grafu")
st.sidebar.caption("Definujte výchozí zobrazení osy X napříč všemi grafy.")

filtered_min_date = cpi_filtered.index.min().date()
filtered_max_date = cpi_filtered.index.max().date()

# Stanovení implicitního přiblížení na posledních 7 let (pokud jsou data k dispozici)
...
# Proměnná definující rozsah osy X, využívaná přímo v grafických objektech Plotly
axis_view_range = [str(zoom_start), str(zoom_end)]

st.sidebar.markdown("---")

# C) Konfigurace bazického roku (Přepočet cenové hladiny na Index = 100)
st.sidebar.subheader("3. Bazický rok (Index = 100)")
st.sidebar.caption("Zvolte referenční rok, vůči kterému se bude porovnávat historický cenový vývoj.")

available_years = sorted(cpi_raw.index.year.unique())
...
# --- JÁDRO VÝPOČTU: PŘEBÁZOVÁNÍ INDEXŮ ---
# 1. Přebázování národního indexu CPI
...
# Matematická transformace: Všechny hodnoty dělíme průměrem referenčního roku a násobíme stem
cpi_base_mean = cpi_base_subset['Úhrn'].mean()
cpi_rebased = (cpi_filtered / cpi_base_mean) * 100

# 2. Přebázování harmonizovaného indexu HICP
# Řada HICP nemusí pokrývat celé období CPI. Ošetřujeme scénář pro případ chybějícího referenčního roku.
hicp_base_subset = hicp_raw[hicp_raw.index.year == base_year]

if not hicp_base_subset.empty:
    hicp_base_mean = hicp_base_subset['Úhrn'].mean()
    hicp_rebased = (hicp_filtered / hicp_base_mean) * 100
else:
    # V případě chybějících dat vygenerujeme prázdný datový rámec (NaN),
    # což zabrání selhání aplikace. Grafy závislé na těchto datech se nevykreslí.
    hicp_rebased = hicp_filtered * np.nan


# --- 4. HLAVNÍ OBSAH APLIKACE ---
st.title("Dashboard pro analýzu inflace")

# Zobrazení klíčových souhrnných metrik pro aktuální datový filtr
...
# Rozdělení uživatelského rozhraní do logických záložek
tab_uvod, tab_bazicky, tab_mezirocni, tab_mezimesicni, tab_vahy, tab_vlastni = st.tabs([
    "🏠 Úvod a manuál",
...
# ==========================================
# ZÁLOŽKA 0: ÚVOD A METODIKA
# ==========================================
# Tato sekce slouží jako textový rozcestník a manuál pro uživatele.
# Neobsahuje žádné analytické výpočty, pouze strukturuje a zobrazuje statický obsah.

with tab_uvod:
    st.header("Vítejte v interaktivním dashboardu pro analýzu inflace v ČR")
    
    st.markdown("""
    Tato aplikace poskytuje přístup k oblasti spotřebitelských cen. Nabízí ucelený a zároveň detailní 
    přehled o vývoji cen zboží a služeb a zároveň o struktuře výdajů průměrných českých domácností.
    

    Aplikace umožňuje sledovat dlouhodobý vývoj jednotlivých kategorií spotřebního koše, 
    poskytuje informace o průměrném růstu, volatilitě a sezónním chování cen a zároveň nabízí 
    možnost výpočtu modelové osobní míry inflace podle zvolených parametrů.
    """)
    
    st.markdown("---")
    
    # Rozdělení obrazovky na dva sloupce pro sekce 'S jakými daty se pracuje?' a 'Jak dashboard ovládat?'
    ...
    with col_info1:
        ...
        * **Srovnání s HICP:** Pro mezinárodní kontext jsou k dispozici také data harmonizovaného indexu spotřebitelských cen (HICP), který je používán v rámci EU a metodicky se liší zejména nezahrnutím imputovaného nájemného.
    ...
    with col_info2:
        ...
        st.markdown("""
        Nastavení najdete v levém postranním panelu. Vaše nastavení se okamžitě propíše do všech grafů:
        ...
        3. **Bazický rok (Index = 100):** Toto je klíčové pro 1. záložku. Vyberete si rok, který bude sloužit jako základní rok (jeho hodnota je stanovena na 100). Pokud si vyberete rok 2015 a křivka dnes ukazuje 150, znamená to, že cenová hladina od té doby stoupla o 50 %.
    ...
    # Rozdělení průvodce obsahem do dvou sloupců pro lepší vizuální oddělení jednotlivých záložek
    guide_col1, guide_col2 = st.columns(2)
    
    with guide_col1:
        st.info("""
        **📈 1. Vývoj cen od určitého roku (Dlouhodobý vývoj)**
        ...
        * **Co zjistíte:** O kolik procent vše podražilo od vašeho referenčního bodu a kolik ze své reálné kupní síly ztratila 1000 Kč bankovka. Najdete zde i detailní pohled na skokové zdražování bydlení.
        ...
        st.error("""
        **📉 3. Meziměsíční inflace (Krátkodobé výkyvy a sezónnost)**
        * **Co tu najdete:** Analýzu změn měsíc po měsíci. Kdy se v roce historicky zdražuje a kdy zlevňuje?
        * **Co zjistíte:** Odhalíte skrytou sezónnost (např. že oblečení zlevňuje ve výprodejích v lednu). Dále zjistíte, jak velká nestabilita se týká jednotlivých roků a kategorií.
        ...
        # Vlastní HTML/CSS stylizace pro vizuální odlišení sekce "Moje inflace"
        st.markdown("""
    ...
    with guide_col2:
        st.success("""
        **🧮 2. Meziroční inflace (Tempo zdražování)**
        ...
        * **Co zjistíte:** Prozkoumáte historické inflační vlny pomocí přehledné teplotní mapy. Zjistíte také, které konkrétní položky táhly inflaci nahoru ve vybraném měsíci a podíváte se na rozdíl mezi českou a evropskou metodikou.
        ...
        st.warning("""
        **⚖️ 4. Váhy spotřebního koše (Struktura útrat)**
        * **Co tu najdete:** Anatomii české peněženky. Za co se vydává nejvíce peněz?
...
with tab_bazicky:
    ...
    kpi1.metric(
        label="📈 Celkový nárůst cen (v období)",
    ...
    kpi2.metric(
        ...
        help=f"Reálná hodnota nominálních 1 000 Kč z počátku sledovaného období ({str_start_date}) k ({str_end_date})."
    )
    ...
    st.subheader(f"1. Celkový vývoj cenové hladiny (Báze {base_year}=100)")
    st.markdown("Základní pohled na úhronnou inflaci. Červená část křivky zvýrazňuje vývoj od vámi zvoleného referenčního roku do současnosti.")
    ...
    st.markdown("Rozpad indexu do 12 hlavních oddílů spotřebního koše. Zde vidíte, jak zdražují důležitější položky (oranžově zvýrazněné kritické položky jako Bydlení a Potraviny) a ty méně důležité (modré).")
    ...
    st.markdown("Kategorie bydlení je velmi diskutovaná. Schodový graf ukazuje, že změny cen nejsou zaznamenávány neustále, ale měsíčně. Díky schodům lze snadněji identifikovat znatelnější zdražení.")
    ...
    st.subheader("4. Metodické srovnání: Národní (CPI) vs. Evropská (HICP) inflace")
    st.markdown("Graf ukazuje rozdíl mezi úhrnným národním indexem (CPI) a harmonizovaným indexem EU (HICP).")
...
# ==========================================
# 2. ZÁLOŽKA: MEZIROČNÍ INFLACE
# ==========================================
with tab_mezirocni:
    st.caption("Analýza vývoje aktuální inflace ve srovnání se stejným měsícem minulého roku.")
    # --- SEKCE 0: KLÍČOVÉ UKAZATELE (KPI) ---
    cpi_yoy_total = cpi_yoy_filtered['Úhrn']
    
    # 1. Výpočet průměrné roční míry inflace (Klouzavý průměr za 12 měsíců)
    # Tento výpočet eliminuje sezónnost tím, že porovnává sumu indexů za posledních 12 měsíců 
    # se sumou indexů za předchozích 12 měsíců (odpovídá metodice ČSÚ).
    latest_date = cpi_yoy_total.index[-1]
    current_pos = cpi_raw.index.get_loc(latest_date)

    if current_pos >= 23:
        # Definice hranic časových oken
        ...
        # Záložní výpočet pro případ, kdy je vybráno příliš krátké období (méně než 2 roky)
    ...
    # 2. Výpočet historických extrémů v rámci vybraného období
    ...
    # 3. Aktuální hodnota a meziroční dynamika (změna tempa)
    ...
    # --- VYKRESLENÍ INFORMAČNÍCH PANELŮ ---
    m_col1, m_col2, m_col3, m_col4 = st.columns(4)

    m_col1.metric(
        "📊 Průměrná roční inflace",
        f"{avg_inflation_val:.1f}".replace('.', ',') + " %",
        help=help_text_avg
    )

    m_col2.metric(
        "🚀 Vrchol (MAX)", 
        f"{max_inf_val:.1f}".replace('.', ',') + " %", 
        delta=max_inf_str, 
        delta_color="off"
    )
    ...
    st.markdown("Rychlý pohled na inflační vlny v historii. Tmavě červená pole značí období vysoké inflace, bíla až modrá pole značí období stability nebo deflace. Osa Y představuje roky, osa X měsíce.")
    ...
    if not cpi_yoy_filtered.empty:
        ...
        # Agregace dat do matice [Rok x Měsíc]
        heatmap_matrix = heatmap_df.pivot(index='Year', columns='Month', values='Úhrn')
        
        hover_matrix = [
            [f"{CZ_MONTHS[m]} {y}" for m in heatmap_matrix.columns] 
            for y in heatmap_matrix.index
        ]

        # Konverze čísel měsíců na české zkratky
        ...
        # Definice dynamického rozsahu osy Y podle uživatelského zoomu
    ...
    st.subheader("2. Detailní vývoj pro jednotlivé kategorie")
    st.markdown("Porovnání vývoje cen v jednotlivých oddílech spotřebního koše. Oranžově jsou zvýrazněny klíčové kategorie (Bydlení, Potraviny), které mají největší dopad na peněženky domácností.")
    ...
    # --- SEKCE 3: ROZPAD INFLACE V KONKRÉTNÍM MĚSÍCI (LOLLIPOP CHART) ---
    st.subheader("3. Rozklad inflace: Který oddíl zdržoval nejvíce nejvíce?")
    st.markdown("Detailní pohled na vybraný měsíc. Zobrazuje inflaci jednotlivých kategorií, a jak se liší od celkového průměru. Červené body značí nadprůměrné zdražování, modré podprůměrné.")
    
    available_dates = cpi_yoy_filtered.index 
    ...
    if len(available_dates) > 0:
        ...
        if selected_date:
            monthly_data = cpi_yoy_filtered.loc[selected_date]
            monthly_total = monthly_data['Úhrn']
            monthly_categories = monthly_data.drop('Úhrn').sort_values()
            
            # Vyčištění názvů odstraněním číselného kódu a zkrácením textu
            ...
            # Dynamické určení barev a formátování tooltipů
            ...
            # Horizontální "tyčky" grafu ukazující odchylku od průměru
            ...
            # Datové "kuličky" určující přesnou hodnotu
    ...
    st.markdown("O kolik procentních bodů se liší česká metodika od evropské? Modré sloupce ukazují, kdy česká inflace převyšuje tu harmonizovanou. Zásadním faktorem odlišnosti je započítávání nákladů na vlastnické bydlení (imputovaného nájemného).")
    ...
    # Bezpečné nalezení patřičných sloupců v obou datasetech
...
with tab_mezimesicni:
    ...
    def geo_mean_pct(series):
        """
        Vypočítá průměrné tempo růstu pomocí geometrického průměru.
        Převádí procentuální změny na faktory (např. +2 % -> 1.02), vypočte průměr a vrátí zpět do formátu procent.
        """
    ...
    # 1. Výpočet průměrného meziměsíčního tempa
    avg_mom_growth = geo_mean_pct(mom_total_series)
    
    # 2. Analýza deflace (pokles cenové hladiny oproti předchozímu měsíci)
    deflation_months_count = (mom_total_series < 0).sum()
    
    # 3. Identifikace největšího cenového skoku v jednom měsíci
    ...
    # Vykreslení KPI metrik
    mm_col1, mm_col2, mm_col3 = st.columns(3)
    
    mm_col1.metric(
        "⚡ Průměrné meziměsíční tempo",
        f"{avg_mom_growth:.2f}".replace('.', ',') + " %",
        help=f"Průměrná změna cenové hladiny za jeden měsíc (vypočteno geometrickým průměrem za období {period_mom_str})."
    )
    ...
    # Agregace dat podle kalendářního měsíce
    ...
    # Centralizované čištění názvů kategorií (odstranění číselných prefixů a zkrácení)
    ...
    # Seřazení kategorií dle obecné důležitosti v koši (uživatelsky nejpřívětivější pořadí)
    importance_order = [
    ...
    st.markdown("Graf ukazuje roční stabilitu cen. Délka vertikální čáry značí absolutní rozdíl mezi nejvyšším a nejnižším meziměsíčním růstem v daném roce. Oranžově jsou zvýrazněny roky s extrémní nestabilitou.")
    
    total_col_name = 'Úhrn' if 'Úhrn' in cpi_mom_filtered.columns else cpi_mom_filtered.columns[0]
    
    # Výpočet základních statistických ukazatelů agregovaných ročně
    volatility_stats = cpi_mom_filtered.groupby(cpi_mom_filtered.index.year)[total_col_name].agg(['min', 'max', 'mean', 'median'])
    ...
    hover_template_volatility = (
        ...
        "Medián: %{customdata[4]:.2f} %<br>"
        "Minimální skok: %{customdata[0]:.2f} %<br>"
        "Maximální skok: %{customdata[1]:.2f} %<br>"
        "<b>Rozpětí (Volatilita): %{customdata[2]:.2f} p.b.</b><extra></extra>"
    )

    # Oddělení dat pro standardní a nestabilní roky za účelem barevného odlišení (segmentované čáry)
    x_stable, y_stable, custom_stable = [], [], []
    x_unstable, y_unstable, custom_unstable = [], [], []

    for year, metrics in volatility_stats.iterrows():
        v_min, v_max, v_spread, v_mean, v_median = metrics['min'], metrics['max'], metrics['spread'], metrics['mean'], metrics['median']
        c_data = [v_min, v_max, v_spread, v_mean, v_median]
        
        # Dynamické barvení: Rozpětí větší než 3 procentní body je považováno za vysoce nestabilní
        if v_spread >= 3.0:
    ...
    stats_data_matrix = volatility_stats[['min', 'max', 'spread', 'mean', 'median']].values
    fig_volatility.add_trace(go.Scatter(
...
# --- SEKCE 3: MAPA RIZIKA (SCATTER PLOT) ---
    ...
    * **Osa X (Růst)**: Svislá čára odpovídá měsíčnímu ekvivalentu inflačního cíle ČNB (0,165 %). Odděluje nízký růst od nadměrného zdražování.
    * **Osa Y (Volatilita)**: Měřena jako mezikvartilové rozpětí (IQR) – odolává extrémům. Vodorovná čára ukazuje medián napříč všemi kategoriemi.
    * **Velikost bublin**: Znázorňuje váhu (významnost) položky ve spotřebním koši.
    ...
    fig_risk.add_vline(
        x=cnb_monthly_target, line_width=1, line_dash="dash", line_color="#1f77b4", opacity=0.5,
        annotation_text="Cíl ČNB (0,165 %)", annotation_position="bottom right",
        annotation_font_size=10, annotation_font_color="#1f77b4"
    )
...
# ==========================================
# 4. ZÁLOŽKA: VÁHY SPOTŘEBNÍHO KOŠE
# ==========================================
with tab_vahy:
    st.caption("Analýza složení spotřebního koše. Za co domácnosti utrácejí nejvíce a jak se priority mění v čase?")

    # --- SEKCE 0: KONTROLA DAT A VÝPOČET KPI ---
    if weights_tree is not None:
        # Základní metriky pro zobrazení v informačních panelech
        total_main_categories = 12
        total_subcategories = len(weights_tree)
        
        # Identifikace položky s nejvyšší vahou v koši (ZDE BYLA CHYBA - opraveno na Weight_Pct)
        ...
        w_col1.metric("📂 Hlavní oddíly", f"{total_main_categories}")
        w_col2.metric("🛒 Počet podkategorií", f"{total_subcategories}")
        w_col3.metric(
            "🏠 Největší výdaj", 
    ...
    # Validace přítomnosti potřebných datových rámců
    if weights_tree is None or weights_history is None:
        ...
        # Definice standardizovaných názvů a fixní barevné palety pro 12 hlavních kategorií ECOICOP
        ecoicop_labels = {
            'E01': 'Potraviny a nealko nápoje', 'E02': 'Alkohol a tabák', 'E03': 'Odívání a obuv',
            'E04': 'Bydlení, voda, energie', 'E05': 'Bytové vybavení', 'E06': 'Zdraví',
            'E07': 'Doprava', 'E08': 'Pošty a telekomunikace', 'E09': 'Rekreace a kultura',
            'E10': 'Vzdělávání', 'E11': 'Stravování i ubytování', 'E12': 'Ostatní zboží a služby'
        }
        
        # Vizuálně rozlišitelná paleta pro lepší čitelnost grafů
        distinct_palette = [
        ...
        # --- SEKCE 1: STROMOVÁ MAPA (TREEMAP) ---
        st.subheader("1. Detailní struktura spotřebního koše (2025)")
        st.markdown("Hierarchický pohled na aktuální složení koše. Velikost obdélníku odpovídá váze dané kategorie ve výdajích domácností. Čím větší plocha, tím více peněz průměrná domácnost za položku utratí. Hlavní kategorie jsou odlišeny barvami.") 

        

        MAIN_FONT_SIZE = "18px" # Fixní velikost písma aplikovaná pouze na nadřazené kategorie

        def wrap_text_labels(text, max_width=18):
            """Zalomí dlouhé textové řetězce pomocí HTML tagu <br> pro lepší zobrazení v obdélnících."""
            if not isinstance(text, str): return text
            return "<br>".join(textwrap.wrap(text, width=max_width))

        def parse_main_category_code(code_str):
            """Extrahuje hlavní dvoumístný kód ECOICOP z podrobnějšího kódu (např. '01.1.2' -> 'E01')."""
        ...
        # Zpracování dat pro hierarchickou strukturu
        weights_tree['Parsed_Main_Code'] = weights_tree['ECOICOP'].apply(parse_main_category_code)
        
        # Filtrace: Ponecháme pouze položky nejnižší úrovně (obsahují tečku v kódu ECOICOP)
        ...
        # Přiřazení srozumitelných názvů hlavním kategoriím a příprava formátování pro Plotly
        ...
        st.markdown("""
        Plošný graf zobrazuje změnu váhy jednotlivých kategorií v průběhu let. 
        ...
        if base_year_col and target_year_col:
            ...
            COLOR_STEM_INC = '#f4bcbc'       # Světle červená pro spojovací linku (stonek)
            COLOR_STEM_DEC = '#a8cce8'       # Světle modrá pro spojovací linku (stonek)
            ...
            # Vykreslení spojovacích linek (stonků lízátek)
            ...
            # Vykreslení koncových bodů (hlaviček lízátek)
            fig_lollipop.add_trace(go.Scatter(
...
with tab_vlastni:
    ...
    # 1. Izolace datových sloupců reprezentujících jednotlivé kategorie (vyloučení úhrnu)
    category_columns = [col for col in cpi_raw.columns if col != 'Úhrn']
    
    # 2. Definice srozumitelnějších názvů a pomocných textů (nápověd) dle klasifikace COICOP
    coicop_names_map = {
    ...
    coicop_help_texts = {
        ...
        '04': "Nájemné, elektřina, plyn, voda, teplo, odvoz odpadu, údržba bytu (náklady na hypotéku se nazapočítávají).",
        '05': "Nábytek, koberce, spotřebiče, nádobí, mycí prostředky.",
        '06': "Léky, doplatky u lékaře, zubař, zdravotnické pomůcky.",
        '07': "Nákup aut, benzín/nafta, opravy aut, jízdenky na vlak/bus, letenky.",
        '08': "Mobilní tarify, internet, poštovní služby.",
    ...
    # Pomocná funkce pro unifikaci textových názvů na dvoumístné COICOP kódy
    ...
    # 3. Načtení referenčních vah ČSÚ pro aktuální srovnání (rok 2025)
    csu_weights_2025 = {}
    ...
    # Záložní data (fallback) pro případ selhání importu z Excelu
    if not csu_weights_2025 or len(csu_weights_2025) < 12:
        csu_weights_2025 = {
            '01': 17.8, '02': 8.6, '03': 4.1, '04': 26.4, '05': 5.9, '06': 2.8, 
            '07': 10.4, '08': 2.9, '09': 8.4, '10': 0.6, '11': 6.2, '12': 5.9
        }

    total_csu_weight = sum(csu_weights_2025.values())

    # Výchozí předvyplněné hodnoty reprezentující odhadovanou průměrnou spotřebu
    default_monthly_expenses = {
        '01': 7000, '02': 3500, '03': 1600, '04': 10500, '05': 2400, '06': 1100,
        '07': 4200, '08': 1200, '09': 3400, '10': 300, '11': 2500, '12': 2300
    }

    # 4. Agregace metadat (názvy, nápovědy, defaultní útraty, reálné váhy) k datovým sloupcům
    category_metadata = {}
    for col in category_columns:
        ...
        # Matematický přepočet absolutní váhy z koše ČSÚ na přesný procentní podíl
    ...
    st.subheader("1. Nastavení osobních výdajů")
    st.markdown("Zadejte svou odhadovanou měsíční útratu v Kč pro jednotlivé kategorie (pro zobrazení příkladů položek najeďte myší na ikonu otazníku). Aplikace si tyto částky automaticky přepočítá na procentuální váhy vašeho osobního spotřebního koše.")
    
    user_input_spends = {}
    
    # Použití st.form zabraňuje neustálému přenačítání grafů při každém stisku klávesy v inputu
    with st.form("form_personal_inflation"):
        ...
        # ZMĚNA: Použito type="secondary" pro neutrální barvu tlačítka
    ...
    if total_monthly_spend == 0:
        ...
        # 1. Přepočet nominálních výdajů na procentní váhy osobního koše
        personal_weights_pct = {cat: (spend / total_monthly_spend) * 100.0 for cat, spend in user_input_spends.items()}

        # 2. Výpočet absolutního osobního indexu
        personal_raw_index = pd.Series(0.0, index=cpi_raw.index)
        for cat in category_columns:
            # Každý bazický index kategorie vynásobíme její odpovídající vahou v osobním koši
            personal_raw_index += cpi_raw[cat] * (personal_weights_pct[cat] / 100.0)
            
        # 3. Přebázování indexu vůči referenčnímu roku vybranému v sidebaru
        personal_base_mean = personal_raw_index[personal_raw_index.index.year == base_year].mean()
        personal_index_rebased = (personal_raw_index / personal_base_mean) * 100
        
        # 4. Výpočet dynamiky (meziroční inflace) z osobního indexu
        personal_yoy = personal_raw_index.pct_change(12) * 100
        
        # Aplikace časových filtrů definovaných v postranním panelu
        ...
        st.markdown(f"Graf ukazuje změnu cenové hladiny vůči vybranému základnímu roku ({base_year}=100). Zde můžete sledovat, zda vaše konkrétní struktura výdajů v dlouhodobém horizontu zdražila více než byl celostátní průměr.")
        ...
        st.subheader("4. Srovnání struktury koše (Průměr ČR vs. Vy)")
